...
UI の文字と文言（読みやすさ）に関する UX 規則をチェックし適用するエージェントスキル `ux-readability-review` を管理する。
スキルの形式は Claude Code と Codex で共通であり、どちらか一方に依存する記述を入れない。
スキル本体は `skills/ux-readability-review/` にあり、利用者はこのディレクトリを使うエージェントのスキルディレクトリへコピーして使う。
...
- 「対象外」と「別の表現で満たす条件」を混ぜない。前者はその問題が起きないか、起きても利用者が困らない場合、後者は別の手段で目的を達成できる場合である。
- 規則の一覧表は `skills/ux-readability-review/SKILL.md` と `README.md` の両方にある。片方を変えたらもう一方も揃える。
- 規則を変更したら `skills/ux-readability-review/evals/evals.json` に、その変更を検証できるテストプロンプトを追加する。
